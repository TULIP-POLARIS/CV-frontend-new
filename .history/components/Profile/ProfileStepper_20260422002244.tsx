  ...
  const loadAll = async () => {
    ...
    try {
      ...
      if (personal) {
        ...
        // 📌 FIXED picture mapping
        if (personal.profilePictureUrl) {
          setAvatarUri(personal.profilePictureUrl);
        }
      }

      setEducationData(education?.length ? education : [emptyEducation()]);
      setWorkData(work?.length ? work : [emptyWork()]);
      setSkillsData(skills?.length ? skills : [emptySkill()]);
      setLanguagesData(languages?.length ? languages : [emptyLanguage()]);
    } catch (e) {
