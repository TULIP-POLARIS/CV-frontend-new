...
const emptyItem = (): WorkItem => ({
  id: Date.now().toString(), jobTitle: '', company: '', location: '',
  startDate: '', endDate: '', isCurrent: false, description: '',
});

export default function WorkStep({ data, onChange, onDelete }: Props) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.sectionHeader}>
        ...
        <Text style={styles.sectionTitle}>{t('work.title')}</Text>
      </View>

      {data.map((item, index) => (
        <View key={item.id} style={styles.card}>
          <View style={styles.cardHeader}>
            <Text style={styles.cardTitle}>{t('work.item')} {index + 1}</Text>
            {data.length > 1 && (
          ...
          <Text style={styles.label}>{t('work.jobTitle')} <Text style={styles.required}>*</Text></Text>
          <View style={styles.inputRow}>
            <Icon name="person-outline" size={16} color="#90a4ae" style={styles.inputIcon} />
            <TextInput style={styles.inputFlex} placeholder={t('work.jobTitlePlaceholder')}
              placeholderTextColor="#b0bec5" value={item.jobTitle}
              onChangeText={v => handleChange(item.id, 'jobTitle', v)} autoCapitalize="words" />
          </View>

          <Text style={styles.label}>{t('work.company')} <Text style={styles.required}>*</Text></Text>
          <View style={styles.inputRow}>
            <Icon name="business-outline" size={16} color="#90a4ae" style={styles.inputIcon} />
            <TextInput style={styles.inputFlex} placeholder={t('work.companyPlaceholder')}
              placeholderTextColor="#b0bec5" value={item.company}
              onChangeText={v => handleChange(item.id, 'company', v)} autoCapitalize="words" />
          </View>

          <Text style={styles.label}>{t('work.location')}</Text>
          <View style={styles.inputRow}>
            <Icon name="location-outline" size={16} color="#90a4ae" style={styles.inputIcon} />
            <TextInput style={styles.inputFlex} placeholder={t('work.locationPlaceholder')}
              placeholderTextColor="#b0bec5" value={item.location}
              onChangeText={v => handleChange(item.id, 'location', v)} autoCapitalize="words" />
          </View>

          <View style={styles.row}>
            <View style={styles.halfField}>
              <Text style={styles.label}>{t('work.startDate')} <Text style={styles.required}>*</Text></Text>
            ...
            <View style={styles.halfField}>
              <Text style={styles.label}>{t('work.endDate')}</Text>
              <TouchableOpacity
                style={[styles.inputRow, item.isCurrent && { opacity: 0.5 }]}
                onPress={() => !item.isCurrent && setActivePicker({ itemId: item.id, field: 'endDate' })}
                disabled={item.isCurrent}
              >
                <Icon name="calendar-outline" size={16} color="#90a4ae" style={styles.inputIcon} />
                <Text style={[styles.inputFlex, !item.endDate && { color: '#b0bec5' }]}>
                  {item.isCurrent ? t('work.present') : item.endDate || 'MM/YYYY'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <TouchableOpacity style={styles.currentRow} onPress={() => handleChange(item.id, 'isCurrent', !item.isCurrent)}>
            <View style={[styles.checkbox, item.isCurrent && styles.checkboxActive]}>
              {item.isCurrent && <Icon name="checkmark" size={12} color="#fff" />}
            </View>
            <Text style={styles.currentText}>{t('work.currentlyWorking')}</Text>
          </TouchableOpacity>

          <Text style={styles.label}>{t('work.description')}</Text>
      ...
      <TouchableOpacity style={styles.addBtn} onPress={() => onChange([...data, emptyItem()])}>
        ...
        <Text style={styles.addBtnText}>{t('work.addAnother')}</Text>
      </TouchableOpacity>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  checkbox: { width: 20, height: 20, borderRadius: 5, borderWidth: 2, borderColor: '#3d6fd8', backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center' },
