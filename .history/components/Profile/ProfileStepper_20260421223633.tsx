...
const validateSkills = (items: SkillItem[]): boolean =>
  items.every(i => !i.name || i.name);

const validateLanguages = (items: LanguageItem[]): boolean =>
  items.every(i => !i.language || i.language);
...
export default function ProfileStepper({ currentStep, onStepChange }: Props) {
    ...
    try {
      ...
      if (personal.status === 'fulfilled' && personal.value) {
        ...
        setPersonalData({
          ...
          nationality: p.nationality ?? '', address: p.address ?? '',
          countryOfResidence: p.countryOfResidence ?? '', phoneNumber: p.phoneNumber ?? '',
        });
      }
  ...
  const handleNext = async () => {
    ...
    // step 4 - languages (last step)
    if (currentStep === 4) {
      if (isLanguageFilled(languagesData)) {
        setSaving(true);
        try { await saveLanguages(); } catch { /* ignore */ } finally { setSaving(false); }
      }
